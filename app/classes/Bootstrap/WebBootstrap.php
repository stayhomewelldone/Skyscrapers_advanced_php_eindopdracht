...
use Buildings\Handlers\BaseHandler;
use Buildings\Utils\Session;
use Buildings\Utils\Logger;
...
class WebBootstrap implements BootstrapInterface
{
    ...
    public function setup(): void
    {
        ...
        if (isset($routes[$route])) {
            ...
            $this->className = '\\Buildings\\Handlers\\' . $class;
            $this->action = $action;
        } else {
            header('HTTP/1.0 404 Not Found');
            $this->className = '\\Buildings\\Handlers\\NotFoundHandler';
            $this->action = 'index';
        }
    }
